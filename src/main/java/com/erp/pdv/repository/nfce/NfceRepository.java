package com.erp.pdv.repository.nfce;

import java.util.List;
import java.util.Optional;

import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.EntityGraph;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import com.erp.pdv.model.nfce.Nfce;
import com.erp.pdv.projections.NfceMinProjection;

public interface NfceRepository extends JpaRepository<Nfce, Long> {

    /**
     * Busca uma NFC-e pelo ID já carregando os itens (itens não virão lazy).
     */
    @EntityGraph(attributePaths = "itens")
    Optional<Nfce> findWithItensById(Long id);
    /*
     * @Query(value = """
     * SELECT
     * n.id AS id,
     * n.serie AS serie,
     * n.chave AS chave,
     * n.numero_nf AS numeroNf,
     * n.ambiente AS ambiente,
     * n.status AS status,
     * n.valor_total_nota AS valorTotalNota,
     * n.data_hora_emissao AS dataHoraEmissao,
     * n.motivo_status AS motivoStatus,
     * n.desconto AS desconto,
     * 
     * -- Emitente
     * e.id AS emitenteId,
     * e.x_nome AS emitenteNome,
     * e.cnpj AS emitenteCnpj,
     * 
     * -- Destinatário
     * d.id AS destinatarioId,
     * d.x_nome AS destinatarioNome,
     * d.cpf AS destinatarioCpf,
     * d.cnpj AS destinatarioCnpj,
     * 
     * -- Pagamento
     * p.id AS pagId,
     * p.t_pag AS pagamentoTipo,
     * 
     * -- Itens
     * i.product_id AS productId,
     * i.codigo_principal AS itemCodigoPrincipal,
     * i.descricao AS itemDescricao,
     * i.q_com AS itemQCom,
     * i.preco_venda AS itemPrecoVenda
     * 
     * FROM tb_nfce n
     * INNER JOIN tb_emitente e ON n.empresa_id = e.id
     * LEFT JOIN tb_destinatario d ON n.destinatario_id = d.id
     * LEFT JOIN tb_pagamento p ON n.pagamento_id = p.id
     * LEFT JOIN tb_item_nfce i ON i.nfce_id = n.id
     * 
     * WHERE
     * -- FILTRO DE DATA OPCIONAL
     * (:minDate IS NULL OR :maxDate IS NULL
     * OR (n.data_hora_emissao IS NOT NULL
     * AND n.data_hora_emissao BETWEEN CAST(:minDate AS timestamp) AND CAST(:maxDate
     * AS timestamp)))
     * 
     * -- FILTRO DE cpf OPCIONAL
     * AND (:destinatarioId IS NULL OR d.id = :destinatarioId)
     * 
     * -- FILTRO DE CNPJ OPCIONAL
     * AND (:emitenteId IS NULL OR e.id = :emitenteId)
     * 
     * -- INCLUI ORÇAMENTOS COM data_hora_emissao NULL
     * AND (n.data_hora_emissao IS NOT NULL OR :minDate IS NULL OR :maxDate IS NULL)
     * 
     * ORDER BY
     * n.data_hora_emissao DESC NULLS LAST, -- Primeiro por data (NULLs no final)
     * n.id DESC, -- Depois por ID (ordem de criação)
     * i.product_id
     * """, countQuery = """
     * SELECT COUNT(DISTINCT n.id)
     * FROM tb_nfce n
     * INNER JOIN tb_emitente e ON n.empresa_id = e.id
     * LEFT JOIN tb_destinatario d ON n.destinatario_id = d.id
     * LEFT JOIN tb_pagamento p ON n.pagamento_id = p.id
     * WHERE
     * (:minDate IS NULL OR :maxDate IS NULL
     * OR (n.data_hora_emissao IS NOT NULL
     * AND n.data_hora_emissao BETWEEN CAST(:minDate AS timestamp) AND CAST(:maxDate
     * AS timestamp)))
     * AND (:destinatarioId IS NULL OR d.id = :destinatarioId)
     * AND (:emitenteId IS NULL OR e.id = :emitenteId)
     * AND (n.data_hora_emissao IS NOT NULL OR :minDate IS NULL OR :maxDate IS NULL)
     * """, nativeQuery = true)
     * List<NfceMinProjection> findAllNfce(
     * 
     * @Param("minDate") String minDate,
     * 
     * @Param("maxDate") String maxDate,
     * 
     * @Param("destinatarioId") Long destinatarioId,
     * 
     * @Param("emitenteId") Long emitenteId);
     */

    /*
     * // Query 1 — só IDs, paginável corretamente
     * // Query 1 — IDs paginados com COALESCE para NULLs ficarem no topo
     * 
     * @Query(value = """
     * SELECT DISTINCT n.id, COALESCE(n.data_hora_emissao, '9999-12-31 23:59:59') AS
     * ordem
     * FROM tb_nfce n
     * INNER JOIN tb_emitente e ON n.empresa_id = e.id
     * LEFT JOIN tb_destinatario d ON n.destinatario_id = d.id
     * WHERE
     * (:minDate IS NULL OR :maxDate IS NULL
     * OR (n.data_hora_emissao IS NOT NULL
     * AND n.data_hora_emissao BETWEEN CAST(:minDate AS timestamp) AND CAST(:maxDate
     * AS timestamp)))
     * AND (:destinatarioId IS NULL OR d.id = :destinatarioId)
     * AND (:emitenteId IS NULL OR e.id = :emitenteId)
     * AND (n.data_hora_emissao IS NOT NULL OR :minDate IS NULL OR :maxDate IS NULL)
     * ORDER BY COALESCE(n.data_hora_emissao, '9999-12-31 23:59:59') DESC, n.id DESC
     * """, countQuery = """
     * SELECT COUNT(DISTINCT n.id)
     * FROM tb_nfce n
     * INNER JOIN tb_emitente e ON n.empresa_id = e.id
     * LEFT JOIN tb_destinatario d ON n.destinatario_id = d.id
     * WHERE
     * (:minDate IS NULL OR :maxDate IS NULL
     * OR (n.data_hora_emissao IS NOT NULL
     * AND n.data_hora_emissao BETWEEN CAST(:minDate AS timestamp) AND CAST(:maxDate
     * AS timestamp)))
     * AND (:destinatarioId IS NULL OR d.id = :destinatarioId)
     * AND (:emitenteId IS NULL OR e.id = :emitenteId)
     * AND (n.data_hora_emissao IS NOT NULL OR :minDate IS NULL OR :maxDate IS NULL)
     * """, nativeQuery = true)
     * Page<Long> findNfceIds(
     * 
     * @Param("minDate") String minDate,
     * 
     * @Param("maxDate") String maxDate,
     * 
     * @Param("destinatarioId") Long destinatarioId,
     * 
     * @Param("emitenteId") Long emitenteId,
     * Pageable pageable);
     * 
     * // Query 2 — dados completos dos IDs da página, mesma ordenação
     * 
     * @Query(value = """
     * SELECT
     * n.id AS id, n.serie AS serie, n.chave AS chave,
     * n.numero_nf AS numeroNf, n.ambiente AS ambiente,
     * n.status AS status, n.valor_total_nota AS valorTotalNota,
     * n.data_hora_emissao AS dataHoraEmissao, n.motivo_status AS motivoStatus,
     * n.desconto AS desconto,
     * e.id AS emitenteId, e.x_nome AS emitenteNome, e.cnpj AS emitenteCnpj,
     * d.id AS destinatarioId, d.x_nome AS destinatarioNome,
     * d.cpf AS destinatarioCpf, d.cnpj AS destinatarioCnpj,
     * p.id AS pagId, p.t_pag AS pagamentoTipo,
     * i.product_id AS productId, i.codigo_principal AS itemCodigoPrincipal,
     * i.descricao AS itemDescricao, i.q_com AS itemQCom,
     * i.preco_venda AS itemPrecoVenda
     * FROM tb_nfce n
     * INNER JOIN tb_emitente e ON n.empresa_id = e.id
     * LEFT JOIN tb_destinatario d ON n.destinatario_id = d.id
     * LEFT JOIN tb_pagamento p ON n.pagamento_id = p.id
     * LEFT JOIN tb_item_nfce i ON i.nfce_id = n.id
     * WHERE n.id IN (SELECT unnest(CAST(:ids AS bigint[])))
     * ORDER BY COALESCE(n.data_hora_emissao, '9999-12-31 23:59:59') DESC, n.id
     * DESC, i.product_id
     * """, nativeQuery = true)
     * List<NfceMinProjection> findNfceByIds(@Param("ids") List<Long> ids);
     * 
     */

    // Query 1 — só IDs, paginável corretamente
    @Query(value = """
            SELECT
                n.id,
                SUM(n.valor_total_nota) OVER() AS total_periodo
            FROM tb_nfce n
            WHERE (:emitenteId IS NULL OR n.empresa_id = :emitenteId)
            AND (:destinatarioId IS NULL OR n.destinatario_id = :destinatarioId)
            AND (:minDate IS NULL OR n.data_hora_emissao >= :minDate)
            AND (:maxDate IS NULL OR n.data_hora_emissao <= :maxDate)
            ORDER BY n.data_hora_emissao DESC
            """, countQuery = """
            SELECT COUNT(n.id)
            FROM tb_nfce n
            WHERE (:emitenteId IS NULL OR n.empresa_id = :emitenteId)
            AND (:destinatarioId IS NULL OR n.destinatario_id = :destinatarioId)
            AND (:minDate IS NULL OR n.data_hora_emissao >= :minDate)
            AND (:maxDate IS NULL OR n.data_hora_emissao <= :maxDate)
            """, nativeQuery = true)
    Page<Object[]> findNfceIdsComTotal(
            @Param("minDate") String minDate,
            @Param("maxDate") String maxDate,
            @Param("destinatarioId") Long destinatarioId,
            @Param("emitenteId") Long emitenteId,
            Pageable pageable);

    // Query 2 — dados completos só dos IDs da página
    @Query(value = """
            SELECT
                n.id AS id, n.serie AS serie, n.chave AS chave,
                n.numero_nf AS numeroNf, n.ambiente AS ambiente,
                n.status AS status, n.valor_total_nota AS valorTotalNota,
                n.data_hora_emissao AS dataHoraEmissao, n.motivo_status AS motivoStatus,
                n.desconto AS desconto,
                e.id AS emitenteId, e.x_nome AS emitenteNome, e.cnpj AS emitenteCnpj,
                d.id AS destinatarioId, d.x_nome AS destinatarioNome,
                d.cpf AS destinatarioCpf, d.cnpj AS destinatarioCnpj,
                p.id AS pagId, p.t_pag AS pagamentoTipo,
                i.product_id AS productId, i.codigo_principal AS itemCodigoPrincipal,
                i.descricao AS itemDescricao, i.q_com AS itemQCom,
                i.preco_venda AS itemPrecoVenda
            FROM tb_nfce n
            INNER JOIN tb_emitente e ON n.empresa_id = e.id
            LEFT JOIN tb_destinatario d ON n.destinatario_id = d.id
            LEFT JOIN tb_pagamento p ON n.pagamento_id = p.id
            LEFT JOIN tb_item_nfce i ON i.nfce_id = n.id
            WHERE n.id IN (:ids)
            ORDER BY n.data_hora_emissao DESC NULLS LAST, n.id DESC, i.product_id
            """, nativeQuery = true)
    List<NfceMinProjection> findNfceByIds(@Param("ids") List<Long> ids);
}
